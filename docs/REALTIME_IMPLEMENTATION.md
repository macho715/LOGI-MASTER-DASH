...
- Schema-qualified tables (`"case".*`, `status.*`) require RLS policies (see `docs/realtime-config-review.md`)
...
### RLS Policies
Ensure RLS policies exist for subscribed tables (see `docs/realtime-config-review.md` for details).
...
- **Project Docs**:
  ...
  - `docs/realtime-config-review.md` (RLS and Realtime configuration)
